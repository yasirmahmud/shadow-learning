export default async function handler(req, res) {
  ...
  try {
    ...
    if (!body || !body.course || !body.answers) return res.status(400).send("Invalid payload. Expect {course, savedAt?, answers}");
    ...
    const ghRes = await fetch(apiUrl, { method:"PUT", headers:{ "Authorization":`Bearer ${token}`,"Content-Type":"application/json","Accept":"application/vnd.github+json","X-GitHub-Api-Version":"2022-11-28" }, body: JSON.stringify({ message:`Save answers for ${body.course} at ${ts}`, content: Buffer.from(contentStr,"utf8").toString("base64"), branch }) });
